...
function normalizeRif(rif) {
  ...
  // Convertir a string y eliminar absolutamente todo lo que NO sea un número
  let cleaned = rif.toString().replace(/[^0-9]/g, "");
...
const proveedoresModel = {
    ...
    return new Promise((resolve, reject) => {
      ...
      pool.query(queryRegistros, [limite, offset], (errReg, resReg) => {
        ...
        pool.query(queryConteo, [], (errCont, resCont) => {
          ...
          const totalRows =
            resCont.rows[0] && resCont.rows[0].total
  ...
  createProveedor(proveedorData) {
    return new Promise((resolve, reject) => {
      const rawRif = proveedorData.rif
        ? proveedorData.rif.toString().toUpperCase()
        : "";

      // 1. Determinar el tipo de documento de forma segura antes de limpiar los números
      // Prioriza el campo explícito, de lo contrario busca si el string del RIF empieza con una letra
      let tipoDocumento = proveedorData.tipo_documento || "J";
      if (
        !proveedorData.tipo_documento &&
        rawRif.length > 0 &&
        /[A-Z]/.test(rawRif[0])
      ) {
        tipoDocumento = rawRif[0];
      }

      // 2. Normalizar el RIF para que sea estrictamente numérico (ej: "504782910")
      const rifValue = normalizeRif(rawRif) || "0";
  ...
  updateProveedorCompleto(id, datos) {
    return new Promise((resolve, reject) => {
      ...
      const rifLimpio = this.normalizeRif
        ? this.normalizeRif(datos.rif)
        : datos.rif;
  ...
  getProveedorByName(razon_social) {
    return new Promise((resolve, reject) => {
      pool.query(
        "SELECT * FROM proveedores WHERE razon_social ILIKE $1 LIMIT 1",
        [razon_social],
        (error, results) => {
          if (error) reject(error);
          else resolve(results.rows[0] || null);
